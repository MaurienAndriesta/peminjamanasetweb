    ...
    <div class="bg-white p-6 rounded-xl shadow-lg">
        ...
        <form method="POST" enctype="multipart/form-data" id="editForm">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div class="space-y-6">
                    <?php if (!empty($ruangan_data['gambar'])): ?>
                    <div class="bg-blue-50 text-blue-800 p-3 rounded-lg text-sm">
                        📷 Gambar saat ini: <?= htmlspecialchars($ruangan_data['gambar']) ?>
                    </div>
                    <?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (img.src && img.src.includes('assets/images/') && img.src.length > 30) {
        // Jika ada gambar lama yang valid, sembunyikan placeholder
        img.style.display = 'block';
        placeholder.style.opacity = '0';
    } else {
        // Jika tidak ada gambar, pastikan placeholder terlihat
