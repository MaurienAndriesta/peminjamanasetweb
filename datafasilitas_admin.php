...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    try {
        $db->query("UPDATE fasilitas SET status = 'tidak_aktif' WHERE id = :id"); 
        $db->bind(':id', $id);
        if ($db->execute()) {
            header('Location: datafasilitas_admin.php?status=success_delete');
            exit;
        } else {
            $error_message = "Gagal menghapus data fasilitas";
        }
    } catch (Exception $e) {
...
try {
    ...
    $db->query("SELECT COUNT(*) as total FROM fasilitas WHERE status = 'aktif'" . $search_query);
    if (!empty($search)) {
...
<div id="mainContent" class="main flex-1 p-3 sm:p-5 transition-all duration-300 lg:ml-16 min-h-screen">
    ...
    <div class="flex justify-between items-center mb-4 sm:mb-6">
        ...
        <div class="text-sm text-gray-600 hidden sm:block">
            <span class="font-semibold">Data Fasilitas</span>
        </div>
    </div>
    ...
    <div class="bg-white p-4 sm:p-6 rounded-xl shadow-lg">
        ...
        <div class="mb-4 sm:mb-6">
            ...
            <p class="text-xs sm:text-sm text-gray-600 mt-1">Kelola data fasilitas kampus</p>
        </div>
        
        <!-- Action Bar -->
        <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center mb-4 sm:mb-6 gap-3">
            <a href="tambahdatafasilitas_admin.php" 
        ...
        <div class="bg-white rounded-xl overflow-hidden shadow-md border border-gray-200">
            <?php if (empty($fasilitas_data)): ?>
                ...
                <div class="text-center p-8 sm:p-16">
                    ...
                    <?php if (!empty($search)): ?>
                        <h3 class="text-lg sm:text-xl font-bold text-gray-700 mb-2">Tidak ada fasilitas ditemukan</h3>
                    ...
                    <?php else: ?>
                        <h3 class="text-lg sm:text-xl font-bold text-gray-700 mb-2">Belum ada data fasilitas</h3>
                        <p class="text-sm sm:text-base text-gray-600 mb-4">Tambahkan fasilitas pertama Anda</p>
                        <a href="tambahdatafasilitas_admin.php" 
                           class="bg-amber-500 hover:bg-amber-600 text-gray-900 font-semibold px-4 sm:px-6 py-2 sm:py-3 rounded-lg inline-block transition-colors shadow-md text-sm sm:text-base">
                            ➕ Tambah Fasilitas
                        </a>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-700 text-white">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">Nama Fasilitas</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900"><?= htmlspecialchars($f['nama']) ?></td>
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm">
                                    <div class="flex gap-2 justify-center">
                                        <a href="editdatafasilitas_admin.php?id=<?= $f['id'] ?>" 
                    ...
                    <div class="bg-white border-2 border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
                        <div class="flex justify-between items-start mb-3">
                            <div class="flex-1">
                                ...
                                <h3 class="font-bold text-base text-gray-900 mb-2"><?= htmlspecialchars($f['nama']) ?></h3>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2 mb-3">
                            ...
                            <div class="grid grid-cols-2 gap-2 text-xs">
                                <div class="bg-green-50 p-2 rounded-lg">
                                    ...
                                    <div class="font-bold text-green-600">Rp <?= number_format($f['tarif_internal']??0,0,',','.') ?></div>
                                </div>
                                <div class="bg-blue-50 p-2 rounded-lg">
                                    <div class="text-gray-600 mb-1">Tarif Eksternal</div>
                                    <div class="font-bold text-blue-600">Rp <?= number_format($f['tarif_eksternal']??0,0,',','.') ?></div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="text-gray-500 text-xs sm:text-sm mt-3 text-center">
            Menampilkan <?= count($fasilitas_data) ?> dari <?= $total_records ?> total data
...
function confirmDelete(id) {
    Swal.fire({
        ...
        text: 'Fasilitas akan dinonaktifkan dan tidak ditampilkan dalam daftar',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            window.location.href = 'datafasilitas_admin.php?action=delete&id=' + id;
        }
    });
}
